...
let getBaseDisc () =
    let rec getBaseDiscHelper seen seenChildren remaining =
        match remaining with
        | [] ->
            Set.difference seen seenChildren
        | hd :: tl ->
            let m = Regex.Match(hd, @"(\S+) \((\d+)\) -> (.*)")
            if m.Success then
                let children = Set.ofArray (m.Groups.[3].Value.Split([|", "|], StringSplitOptions.RemoveEmptyEntries))
                getBaseDiscHelper (Set.add m.Groups.[1].Value seen) (Set.union children seenChildren) tl
            else
                let m = Regex.Match(hd, @"(\S+) \((\d+)\)")
                if m.Success then
                    getBaseDiscHelper (Set.add m.Groups.[1].Value seen) seenChildren tl
                else
                    getBaseDiscHelper seen seenChildren tl
    getBaseDiscHelper Set.empty Set.empty (List.ofArray <| Input.Split([|"\r"; "\n"; "\r\n"|], StringSplitOptions.RemoveEmptyEntries))

match List.ofSeq <| getBaseDisc () with
| [x] -> printfn "%s" x
| [] -> failwith "Received empty list"
| lst -> failwith <| sprintf "Received %A" lst