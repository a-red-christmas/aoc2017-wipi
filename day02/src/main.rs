...
fn replace_if<T: Copy, F>(old: &Option<T>, new_val: &T, predicate: F) -> Option<T> where F: Fn(&T, &T) -> bool {
    match *old {
        None => Some(*new_val),
        Some(ref old_val) if predicate(old_val, new_val) => Some(*new_val),
        Some(ref old_val) => Some(*old_val)
    }
}
...
fn get_line_value(line: &str) -> i32 {
    ...
    for val in line.split_whitespace() {
        ...
        min = replace_if(&min, &val, |x, y| x > y);
        max = replace_if(&max, &val, |x, y| x < y);
    }

    match (min, max) {
        (Some(min_val), Some(max_val)) if min_val <= max_val => max_val - min_val,
        (None, None) => 0,
        _ => panic!("min and max were {:?} and {:?}", min, max)
    }
}
